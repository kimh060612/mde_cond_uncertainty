...
from model.dav2_camerror_model import _load_checkpoint_state
...
@hydra.main(
    version_base=None,
    config_path="config",
    config_name="comparison_caminduce_metric",
)
def main(cfg: DictConfig) -> None:
    ...
    metric_checkpoint_value = (
        cfg.model.get("metric_checkpoint") or training_cfg.get("metric_checkpoint")
    )
    if not metric_checkpoint_value:
        raise ValueError("model.metric_checkpoint must point to the fine-tuned depth model.")
    metric_checkpoint = Path(
        to_absolute_path(str(metric_checkpoint_value))
    )
    if not metric_checkpoint.exists():
        raise FileNotFoundError(metric_checkpoint)
    model_key = str(cfg.model.model_id)
    base_model_id = MODEL_IDS.get(model_key, model_key)
    is_dav3 = model_key.lower().startswith("da3") or "/da3" in model_key.lower()
    model_id = str(metric_checkpoint) if metric_checkpoint.is_dir() else base_model_id

    image_processor = (
        _DA3ImageProcessor(int(cfg.model.get("dav3_process_res", 504)))
        if is_dav3
        else AutoImageProcessor.from_pretrained(
            str(metric_checkpoint)
            if metric_checkpoint.is_dir()
            and (metric_checkpoint / "preprocessor_config.json").is_file()
            else base_model_id,
            use_fast=False,
        )
    )
    ...
    if is_dav3:
        ...
        if metric_checkpoint.is_file():
            state = _load_checkpoint_state(metric_checkpoint)
            target = model if any(key.startswith("model.") for key in state) else model.model
            target.load_state_dict(state, strict=True)
    else:
        model = AutoModelForDepthEstimation.from_pretrained(model_id)
        if metric_checkpoint.is_file():
            model.load_state_dict(_load_checkpoint_state(metric_checkpoint), strict=True)
