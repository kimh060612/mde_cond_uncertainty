...
class FrozenDepthCameraGaussian(nn.Module):
    """
    Frozen depth foundation model + camera-conditioned Gaussian correction.
    ...
    def forward(
        ...
        base_depth, frozen_feature = (
            self._extract_frozen_outputs(pixel_values)
        )
        ...
        conditioned_feature = self._apply_film(
            shared_feature,
            context,
        )

        camera_bias = self.bias_head(
            conditioned_feature
        )

        if self.max_bias is not None:
            camera_bias = self.max_bias * torch.tanh(
                camera_bias
            )
        ...
        if target_size is None:
            target_size = base_depth.shape[-2:]
